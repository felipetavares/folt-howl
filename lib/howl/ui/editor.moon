...
import Scintilla, Completer, signal, bindings, config, interact,  command, clipboard from howl
...
class Editor extends PropertyObject
  ...
  toggle_selection: =>
    if @cursor.extend_selection
      @cursor.extend_selection = false
    else
      @cursor.extend_selection = true
  ...
  delete_word_back: =>
    ...
    if @selection.empty
      @buffer\delete @cursor.pos, pos-1 if pos != @cursor.pos
    else
      @active_chunk\delete!
  ...
  delete_word_front: =>
    ...
    if @selection.empty
      @buffer\delete pos, @cursor.pos-1 if pos != @cursor.pos
    else
      @active_chunk\delete!
...
for cmd_spec in *{
  ...
  { 'insert-line', 'Insert newline after current line', 'insert_line' }
  { 'insert-line-before', 'Insert newline before current line', 'insert_line_before' }
  { 'toggle-selection', 'Enables/disables selection', 'toggle_selection' }
  { 'delete-word-back', 'Deletes word to the left', 'delete_word_back' }
  { 'delete-word-front', 'Deletes word to the right', 'delete_word_front' }
}
...
for sel_cmd_spec in *{
  ...
  command.register
    ...
    handler: ->
      howl.app.editor.selection[sel_cmd_spec[1]\gsub '-', '_'] howl.app.editor.selection
      howl.app.editor.cursor.extend_selection = false

command.register
  name: 'editor-goto-line',
  description: 'Goes to line'
  input: -> interact.read_text!
  handler: (line) -> howl.app.editor\goto_line(line)
